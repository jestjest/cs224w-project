...
def train(dataset, args):
    ...
    for epoch in range(args.epochs):
        ...
        for train_indices, test_indices in skf.split(x, y):
            ...
            test_acc = test(dataset, model, test_indices) / test_indices.shape[0]
            split_test_acc.append(test_acc)

        total_loss /= num_splits
        print('Loss: ', total_loss)
        split_test_acc = np.array(split_test_acc)
        print('Epoch acc: %0.4f +- %0.4f' %
              (split_test_acc.mean(), split_test_acc.std()))
...
def test(dataset, model, test_indices):
    ...
    with torch.no_grad():
        ...
        pred = model(dataset).max(dim=1)[1]
        label = dataset.y
